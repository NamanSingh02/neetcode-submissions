class Solution:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        for i in range(1,len(nums)):
            # why is this step important? because you can't access the first element of the list, if a key was not created!
            #dict[i]=value to be assigned, but i can't dereference a list from it!
            if (i!=len(nums)-1):
                dict[i]=[None,None]
